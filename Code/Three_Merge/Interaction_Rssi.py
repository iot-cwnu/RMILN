...
from PR_FSCN_Rssi import ExtractNetwork
...
class Interaction(nn.Module):
    def __init__(self, hidden_dim, l, batch_size, n_classes):
        ...
        self.qr_1 = nn.Parameter(torch.randn(1, self.hidden_dim))
        self.qr_1.data.uniform_(-stdv, stdv).renorm_(2, 0, 1e-5).mul_(1e5)
        self.qr_2 = nn.Parameter(torch.randn(1, self.hidden_dim))
        self.qr_2.data.uniform_(-stdv, stdv).renorm_(2, 0, 1e-5).mul_(1e5)
        self.qp = nn.Parameter(torch.randn(1, self.hidden_dim))
        self.qp.data.uniform_(-stdv, stdv).renorm_(2, 0, 1e-5).mul_(1e5)

        # 交互学习模型
        self.D_Self_A = Self_dimMultiHead_Attention1(self.hidden_dim, None, 256, 2, score_function='scaled_dot_product', dropout=0)
        self.R_Self_A_1 = Self_dimMultiHead_Attention1(self.hidden_dim, None, 256, 2, score_function='scaled_dot_product', dropout=0)
        self.R_Self_A_2 = Self_dimMultiHead_Attention1(self.hidden_dim, None, 256, 2, score_function='scaled_dot_product', dropout=0)
        self.P_Self_A = Self_dimMultiHead_Attention1(self.hidden_dim, None, 256, 2, score_function='scaled_dot_product', dropout=0)
        ...
        self.RgD_M_A = M_Attention(self.hidden_dim, score_function='dot_product', dropout=0)
        self.DgR_M_A = M_Attention(self.hidden_dim, score_function='dot_product', dropout=0)
        ...
        self.gru_cell_R_1 = nn.GRUCell(self.hidden_dim, self.hidden_dim, bias=True)
        self.gru_cell_R_2 = nn.GRUCell(self.hidden_dim, self.hidden_dim, bias=True)
        self.gru_cell_P = nn.GRUCell(self.hidden_dim, self.hidden_dim, bias=True)
        # 线性层（4层）
    ...
    def forward(self, D_g, R_g, P_g):
        ...
        qr_1 = self.qr_1.repeat(mb_size, 1)
        qr_2 = self.qr_2.repeat(mb_size, 1)
        qp = self.qp.repeat(mb_size, 1)

        flag_onelayer = False
        VD1 = VD = self.D_Self_A(D_node_feature, qd, flag_onelayer).squeeze(dim=1)
        VP1_1 = VR_1 =self.R_Self_A_1(R_node_feature, qr_1, flag_onelayer).squeeze(dim=1)
        VP1_2 = VR_2 = self.R_Self_A_2(R_node_feature, qr_2, flag_onelayer).squeeze(dim=1)
        VR1 = VP = self.P_Self_A(P_node_feature, qp, flag_onelayer).squeeze(dim=1)
        # flag_onelayer = False
        # 交互学习
        layers = self.l - 1
        for _ in range(0, layers):
            Atten_R2D = self.RgD_M_A(D_node_feature, VR_1).squeeze(dim=1)  # 跨模态注意
            Atten_D2D = self.D_Self_A(D_node_feature, VD, flag_onelayer).squeeze(dim=1)  # 自注意

            Atten_D2R = self.DgR_M_A(R_node_feature, VD).squeeze(dim=1)
            Atten_R2R_1 = self.R_Self_A_1(R_node_feature, VR_1, flag_onelayer).squeeze(dim=1)

            Atten_P2R = self.PgR_M_A(R_node_feature, VP).squeeze(dim=1)
            Atten_R2R_2 = self.R_Self_A_2(R_node_feature, VP, flag_onelayer).squeeze(dim=1)

            Atten_R2P = self.RgP_M_A(P_node_feature, VR_2).squeeze(dim=1)
            Atten_P2P = self.P_Self_A(P_node_feature, VP, flag_onelayer).squeeze(dim=1)

            Atten_D = self.gru_cell_D(Atten_R2D, Atten_D2D)
            Atten_R_1 = self.gru_cell_R_1(Atten_D2R, Atten_R2R_1)
            Atten_R_2 = self.gru_cell_R_2(Atten_P2R, Atten_R2R_2)
            Atten_P = self.gru_cell_P(Atten_R2P, Atten_P2P)

            VD = self.relu(self.norm_bn(self.gru_cell_D(Atten_D, VD)))
            VR_1 = self.relu(self.norm_bn(self.gru_cell_R_1(Atten_R_1, VR_1)))
            VR_2 = self.relu(self.norm_bn(self.gru_cell_R_2(Atten_R_2, VR_2)))
            VP = self.relu(self.norm_bn(self.gru_cell_P(Atten_P, VP)))
        ...
        VP = VP + P_pool
        # 拼接操作
        Km = torch.cat((VD, VR_1, VR_2, VP), dim=1)
        # 线性层(没加激活函数)
        FC1 = self.Linear1(Km)
...
class EIMANN(nn.Module):
    ...
    def forward(self,D_x, R_x, P_x):
        ...
        R_g = self.enn(R_x)
        P_g = self.enn(P_x)
        Output = self.interaction(D_g, R_g, P_g)
        return Output