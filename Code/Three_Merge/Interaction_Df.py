...
from PR_FSCN_Df import ExtractNetwork
...
class Interaction(nn.Module):
    def __init__(self, hidden_dim, l, batch_size, n_classes):
        ...
        self.qd_1 = nn.Parameter(torch.randn(1, self.hidden_dim))
        self.qd_1.data.uniform_(-stdv, stdv).renorm_(2, 0, 1e-5).mul_(1e5)
        self.qd_2 = nn.Parameter(torch.randn(1, self.hidden_dim))
        self.qd_2.data.uniform_(-stdv, stdv).renorm_(2, 0, 1e-5).mul_(1e5)
        ...
        self.D_Self_A_1 = Self_dimMultiHead_Attention1(self.hidden_dim, None, 256, 2, score_function='scaled_dot_product', dropout=0)
        self.D_Self_A_2 = Self_dimMultiHead_Attention1(self.hidden_dim, None, 256, 2, score_function='scaled_dot_product', dropout=0)
        ...
        self.gru_cell_D_1 = nn.GRUCell(self.hidden_dim, self.hidden_dim, bias=True)
        self.gru_cell_D_2 = nn.GRUCell(self.hidden_dim, self.hidden_dim, bias=True)
    ...
    def forward(self, R_g, D_g, P_g):
        ...
        qd_1 = self.qd_1.repeat(mb_size, 1)
        qd_2 = self.qd_2.repeat(mb_size, 1)
        qp = self.qp.repeat(mb_size, 1)

        flag_onelayer = False
        VR = self.R_Self_A(R_node_feature, qr, flag_onelayer).squeeze(dim=1)
        VD_1 =self.D_Self_A_1(D_node_feature, qd_1, flag_onelayer).squeeze(dim=1)
        VD_2 = self.D_Self_A_2(D_node_feature, qd_2, flag_onelayer).squeeze(dim=1)
        VP = self.P_Self_A(P_node_feature, qp, flag_onelayer).squeeze(dim=1)

        # 交互学习
        layers = self.l - 1
        for _ in range(0, layers):
            Atten_D2R = self.DgR_M_A(R_node_feature, VD_1).squeeze(dim=1)  # 跨模态注意
            Atten_R2R = self.R_Self_A(R_node_feature, VR, flag_onelayer).squeeze(dim=1)  # 自注意

            Atten_R2D = self.RgD_M_A(D_node_feature, VR).squeeze(dim=1)
            Atten_D2D_1 = self.D_Self_A_1(D_node_feature, VD_1, flag_onelayer).squeeze(dim=1)

            Atten_P2D = self.PgD_M_A(D_node_feature, VP).squeeze(dim=1)
            Atten_D2D_2 = self.D_Self_A_2(D_node_feature, VD_2, flag_onelayer).squeeze(dim=1)

            Atten_D2P = self.DgP_M_A(P_node_feature, VD_2).squeeze(dim=1)
            Atten_P2P = self.P_Self_A(P_node_feature, VP, flag_onelayer).squeeze(dim=1)

            Atten_R = self.gru_cell_R(Atten_D2R, Atten_R2R)
            Atten_D_1 = self.gru_cell_D_1(Atten_R2D, Atten_D2D_1)
            Atten_D_2 = self.gru_cell_D_2(Atten_P2D, Atten_D2D_2)
            Atten_P = self.gru_cell_P(Atten_D2P, Atten_P2P)

            VR = self.relu(self.norm_bn(self.gru_cell_R(Atten_R, VR)))
            VD_1 = self.relu(self.norm_bn(self.gru_cell_D_1(Atten_D_1, VD_1)))
            VD_2 = self.relu(self.norm_bn(self.gru_cell_D_2(Atten_D_2, VD_2)))
        ...
        VP = VP + P_pool
        # 拼接操作
        Km = torch.cat((VR, VD_1, VD_2, VP), dim=1)
        # 线性层(没加激活函数)
        FC1 = self.Linear1(Km)
...
class EIMANN(nn.Module):
    ...
    def forward(self,D_x, R_x, P_x):
        ...
        D_g = self.enn(D_x)
        P_g = self.enn(P_x)
        Output = self.interaction(R_g, D_g, P_g)
        return Output