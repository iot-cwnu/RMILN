...
from PR_FSCN_Df import RFDataset
from Interaction_Df import EIMANN
...
#创建时间空间特征提取网络，改下优化器
# Rssi_Fixed.mdl  Rssi_trained(98.45665097236633).mdl  Rssi_trained2(98.50203990936279).mdl
model_dict = torch.load('Df_trained(98.45665097236633).mdl')
...
writer = SummaryWriter('../Three_Merge_result/logs_DfFixed')
...
def test(ep):
    ...
    with torch.no_grad():
        ...
        if ep == 0:
            ...
            torch.save(eimAMN.state_dict(), 'Df_Fixed.mdl')
        else:
            if acc > max(best_acc_list):
                torch.save(eimAMN.state_dict(), 'Df_Fixed.mdl')
                best_acc_list.append(acc)
        if ep == 55:
            with open("../Three_Merge_result/Fixed_params_Df.txt", "a+") as f:
                f.write('\n')
