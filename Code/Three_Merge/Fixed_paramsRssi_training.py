...
#创建时间空间特征提取网络，改下优化器
# Rssi_Fixed.mdl  Rssi_trained2(98.63821864128113).mdl  Rssi_trained2(98.50203990936279).mdl
model_dict = torch.load('Rssi_trained2(98.50203990936279).mdl')
...
writer = SummaryWriter('../Three_Merge_result/logs_RssiFixed')
...
def test(ep):
    ...
    with torch.no_grad():
        ...
        if ep == 0:
            ...
            torch.save(eimAMN.state_dict(), 'Rssi_Fixed.mdl')
        else:
            if acc > max(best_acc_list):
                torch.save(eimAMN.state_dict(), 'Rssi_Fixed.mdl')
                best_acc_list.append(acc)
        if ep == 55:
            with open("../Three_Merge_result/Fixed_params_Rssi.txt", "a+") as f:
                f.write('\n')
