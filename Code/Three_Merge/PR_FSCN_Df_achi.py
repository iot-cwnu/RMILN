...
writer = SummaryWriter('../Three_Merge_result/Batch_size/logs_Df128')
...
def test(ep):
    ...
    with torch.no_grad():
        ...
        if ep == 0:
            ...
            torch.save(eimAMN.state_dict(), '../Three_Merge_result/Batch_size/Df128.mdl')
        else:
        ...
        if ep == 55:
            with open("../Three_Merge_result/Batch_size/Df128.txt", "a+") as f:
                f.write('\n')
...
if __name__ == '__main__':
    for epoch in range(0, 56):
        ...
        train(epoch)
        if epoch == 1:
            model_dict = torch.load('../Three_Merge_result/Batch_size/Df128.mdl')
